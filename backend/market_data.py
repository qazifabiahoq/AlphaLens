...
class NewsFetcher:
    """
    Gathers financial news headlines for specific tickers via Yahoo Finance RSS.
    Returns an empty list if the feed is unavailable.
    """
    ...
    def fetch(self, ticker: str, limit: int = 10) -> list:
        """Return up to `limit` headlines for `ticker` from Yahoo Finance RSS. Cached for 15 minutes."""
        ...
        if not headlines:
            log.warning(f"No headlines available for {ticker} — RSS unavailable")
